...
TextView.prototype.prepData = function(chart) {
	...
	switch(chart) {
		case 'scatterChart':
			...
			settings[chart].xAxisTresholds = self.settings.xVarTreshold
			settings[chart].yAxisTresholds = self.settings.yVarTreshold
			settings[chart].radiusAxisTresholds = self.settings.radiusVarTreshold
			self.setRadius()
			self.setColor()
			break
		case 'textChart':
...
TextView.prototype.buildControls = function() {
	...
	var tresholdControl = function(axis){
		var vals = [];
		for(var i =0;i<=100;i++)
			vals.push(i);
		return {
	...
	// X variable treshold
	self.controlSettings['xVarTreshold'] = tresholdControl('x');
	// Y variable treshold
	self.controlSettings['yVarTreshold'] = tresholdControl('y');
	// radius variable treshold
	self.controlSettings['radiusVarTreshold'] = tresholdControl('radius');
