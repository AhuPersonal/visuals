var settings = {
	...
	scatterChart:{
		...
		customUpdate:function(chart){
			var filterBy = function(axis){
				if (!chart.settings[axis+"AxisTresholds"])
					return false;
				if (chart.settings[axis+"AxisTresholds"][0] === 0 && chart.settings[axis+"AxisTresholds"][1] === 100)
					return false;
				var axisKey = axis=="radius"?"radiusValue":axis;
				var data = chart.data.map(function(x){return x[axisKey]});
				data.sort(function(a,b){return a-b;})
				var lowerValue = parseInt((data.length-1)*(chart.settings[axis+"AxisTresholds"][0]/100))
				var upperValue = parseInt((data.length-1)*(chart.settings[axis+"AxisTresholds"][1]/100))
				return {lowerValue:data[lowerValue], upperValue:data[upperValue]};
			}
			var filters = {};
			
			var x = filterBy("x")
			var y = filterBy("y")
			var radiusValue = filterBy("radius")
			if (x)
				filters.x = x;
			if (y)
				filters.y = y;
			if (radiusValue)
				filters.radiusValue = radiusValue;
			chart.settings.data = chart.data.filter(function(x){
				for (var key in filters)
					if (!(x[key] >= filters[key].lowerValue && x[key] <= filters[key].upperValue))
						return false;
				return true;
			});
